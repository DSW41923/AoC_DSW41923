...
def get_max_geode_opened(time_limit, blueprint):
    max_geode_opened = 0
    max_ore_required = max(
        blueprint['ore_robot_costs']['ore'],
        blueprint['clay_robot_costs']['ore'],
        blueprint['obsidian_robot_costs']['ore'],
        blueprint['geode_robot_costs']['ore'])
    ...
    return max_geode_opened


def part_1(input_string):
    blueprints = parse_blueprints(input_string)
    quality_level_sum = 0
    time_limit = 24
    for i, blueprint in enumerate(blueprints):
        if i == 0:
            continue
        max_geode_opened = get_max_geode_opened(time_limit, blueprint)
...
def part_2(input_string):
    blueprints = parse_blueprints(input_string)[:4]
    num = 1
    time_limit = 32
    for i, blueprint in enumerate(blueprints):
        if i == 0:
...
def main():
    ...
    file_input = open('inputs/2022/Input_19.txt', 'r')
